...
export const HeroChainAnimation: React.FC<HeroChainAnimationProps> = ({ chainType }) => {
  ...
  return (
    <div className="w-full max-w-[650px] h-[120px] mx-auto my-4 relative overflow-hidden rounded-xl bg-[var(--panel-strong)]/30 border border-[var(--line)] p-2">
      <svg viewBox="0 0 600 120" className="w-full h-full">
        ...
        {branchPoints.map((x, i) => {
            ...
            return (
              <g key={i} className="animate-pulse">
                <line
                  ...
                  stroke={isLong ? 'var(--c-accent)' : 'var(--c-secondary)'}
                  strokeWidth="3"
                  strokeLinecap="round"
                />
                {isLong && (
            ...
            return (
              <g key={i}>
                <line
                  ...
                  strokeLinecap="round"
                />
                <circle cx={x} cy={y0 - 16} r="4" fill="var(--c-accent)" />
              </g>
            );
          }
            ...
            return (
              <g key={i}>
                <line
                  ...
                  strokeWidth="3"
                />
                <circle cx={x} cy={isUp ? y0 - 18 : y0 + 18} r="5" fill="var(--c-danger)" />
              </g>
            );
          }
          ...
          if (chainType === 'aromatic') {
            ...
            return (
              <g key={i}>
                <line x1={x} y1={y0} x2={x} y2={y0 - 12} stroke="var(--c-secondary)" strokeWidth="2.5" />
                <polygon
          ...
          if (chainType === 'ester') {
            ...
            return (
              <g key={i}>
                <line x1={x} y1={y0} x2={x} y2={y0 - 14} stroke="var(--c-accent)" strokeWidth="3" />
                <circle cx={x} cy={y0 - 14} r="4.5" fill="var(--c-danger)" />
              </g>
            );
          }
      ...
      <div className="absolute bottom-1 right-3 text-[10px] text-[var(--ink-dim)] font-mono">
        شبیه‌سازی تصویری زنجیر پلیمری
      </div>
    </div>
  );
};
